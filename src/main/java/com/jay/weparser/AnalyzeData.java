...
public class AnalyzeData {
    ...
    public static String getDeviceName(String filePath){
    ...
    private static String getDeviceNameDocx(String filePath){
        String deviceName="Not Found Device Name ! ";
        try {
    ...
    private static String getDeviceNameDoc(String filePath) {
    ...
    public static String getDeviceType(String filePath){
        String deviceType="Not Found Device Name ! ";
        String fileExtension=filePath.substring(filePath.lastIndexOf(".") + 1);
        if (fileExtension.equalsIgnoreCase("docx")) {
            deviceType= getDeviceTypeDocx(filePath);
        } else if (fileExtension.equalsIgnoreCase("doc")) {
            deviceType=getDeviceTypeDoc(filePath);
        } else {
            //System.out.println("不支持的文件类型: " + fileExtension);
            throw new IllegalArgumentException("不支持的文件类型: " + fileExtension);
            //else抛出异常后不会执行后面的return，除非return 放在finally里
    ...
    private static String getDeviceTypeDocx(String filePath){
        String deviceType="Not Found Device Type ! ";
        try {
            FileInputStream fis = new FileInputStream(filePath);
            XWPFDocument document = new XWPFDocument(fis);
            ...
            for (XWPFTable table : document.getTables()) {
                for (XWPFTableRow row : table.getRows()) {
                    for(XWPFTableCell cell : row.getTableCells()) {
                        String cellText = cell.getText();
                        if (cellText.trim().equals("保护型号")) {
                            // 找到包含"设备名称"的单元格后，读取其右边一个单元格的内容
                            int cellIndex = row.getTableCells().indexOf(cell);
                            if (cellIndex < row.getTableCells().size() - 1) {
                                XWPFTableCell rightCell = row.getTableCells().get(cellIndex + 1);
                                deviceType = rightCell.getText();
    ...
    private static String getDeviceTypeDoc(String filePath) {
        //必须保证xwpf和hwpf在maven导入包的版本号相同
        String deviceType="Not Found Device Type ! ";
        try {
            FileInputStream fis = new FileInputStream(filePath);
            ...
            while (tableIterator.hasNext()) {   //遍历每个表格
                ...
                for (int rowIndex = 0; rowIndex < table.numRows(); rowIndex++) {
                    ...
                    for (int colIndex = 0; colIndex < row.numCells()-1; colIndex++) {
                        ...
                        if (cellText.trim().equals("保护型号")) {
                            // 找到包含"设备名称"的单元格后，读取其右边一个单元格的内容
                            deviceType = row.getCell(colIndex+1).text();
                            deviceType = deviceType.substring(0,deviceType.length()-1);//多一个特殊字符要去掉
                            System.out.println("保护型号: " + deviceType);
                            return deviceType;
                        }
                    }
    ...
    public static String getDeviceNumber(String filePath){
        String deviceNum="Not Found Device Number ! ";
        String fileExtension=filePath.substring(filePath.lastIndexOf(".") + 1);
        if (fileExtension.equalsIgnoreCase("docx")) {
            deviceNum= getDeviceNumDocx(filePath);
        } else if (fileExtension.equalsIgnoreCase("doc")) {
            deviceNum=getDeviceNumDoc(filePath);
        } else {
            //System.out.println("不支持的文件类型: " + fileExtension);
            throw new IllegalArgumentException("不支持的文件类型: " + fileExtension);
            //else抛出异常后不会执行后面的return，除非return 放在finally里
    ...
    private static String getDeviceNumDocx(String filePath){
        String deviceNum="Not Found Device Number ! ";
        try {
            FileInputStream fis = new FileInputStream(filePath);
            XWPFDocument document = new XWPFDocument(fis);
            ...
            for (XWPFTable table : document.getTables()) {
                for (XWPFTableRow row : table.getRows()) {
                    for(XWPFTableCell cell : row.getTableCells()) {
                        String cellText = cell.getText();
                        if (cellText.trim().equals("定值单编号")) {
                            // 找到包含"设备名称"的单元格后，读取其右边一个单元格的内容
                            int cellIndex = row.getTableCells().indexOf(cell);
                            if (cellIndex < row.getTableCells().size() - 1) {
                                XWPFTableCell rightCell = row.getTableCells().get(cellIndex + 1);
                                deviceNum = rightCell.getText();
                                System.out.println("定值单编号: " + deviceNum);
                                return deviceNum;
                            }
                        }
                    }
    ...
    private static String getDeviceNumDoc(String filePath) {
        //必须保证xwpf和hwpf在maven导入包的版本号相同
        String deviceNum="Not Found Device Name ! ";
        try {
            FileInputStream fis = new FileInputStream(filePath);
            HWPFDocument document = new HWPFDocument(fis);
            //获取文件内容对象
            ...
            while (tableIterator.hasNext()) {   //遍历每个表格
                ...
                for (int rowIndex = 0; rowIndex < table.numRows(); rowIndex++) {
                    ...
                    for (int colIndex = 0; colIndex < row.numCells()-1; colIndex++) {
                        ...
                        if (cellText.trim().equals("定值单编号")) {
                            // 找到包含"设备名称"的单元格后，读取其右边一个单元格的内容
                            deviceNum = row.getCell(colIndex+1).text();
                            deviceNum = deviceNum.substring(0,deviceNum.length()-1);//多一个特殊字符要去掉
                            System.out.println("定值单编号: " + deviceNum);
                            return deviceNum;
                        }
                    }
    ...
    private static Map<String,Double> analyzeDocx(String filePath){
    ...
    private static Map<String,Double> analyzeDoc(String filePath) {
        //必须保证xwpf和hwpf在maven导入包的版本号相同
        Map<String, Double> map = new HashMap<>();    //读取表格内容放在map上
        try {
            FileInputStream fis = new FileInputStream(filePath);
